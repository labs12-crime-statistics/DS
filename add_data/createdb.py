...
def add_incidents_chicago(cityid):
    ...
    incidents = incidents.dropna()
    incident_rows = []
    for i in incidents.index.values:
        incident_rows.append(Incident(crimetypeid=int(incidents.loc[i,"crimetypeid"]), cityid=int(cityid), blockid=int(incidents.loc[i,"blockid"]), location=incidents.loc[i,"locfull"], datetime=datetime.datetime.strptime(incidents.loc[i,"Date"], '%m/%d/%Y %I:%M:%S %p')))
